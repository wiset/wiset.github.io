 ...
 * =====================
 * Goto random hashtag from config list, retweet fast 12 tweet and sleep X min in night.
 *
...
class Rtmode_realistic extends Status {
	constructor() {
		super();
		this.core = core;
		this.LOG_NAME = "rtmode_realistic";
	...
	async flow() {
		...
		let bot_seconds_wait_min = this.utils.compute_interval_between_run_in_seconds(this.core.config.bot_mode_options[this.core.config.bot_mode].rtday_min, 12);
		let bot_seconds_wait_max = this.utils.compute_interval_between_run_in_seconds(this.core.config.bot_mode_options[this.core.config.bot_mode].rtday_max, 12);
		...
		do {
			...
			if (!is_sleep_night_flag_enabled || is_day) {
				...
				if (this.local.rt_counter < 1 || this.local.cache_tweets_urls <= 0) {
				...
				await this.api.goto.url(this.local.current_tweet_urls);

				await this.utils.sleep(this.utils.random_interval(1, 3));

				let response_retweet = await this.api.click.retweet();

				if (response_retweet.status) {
					this.local.rt_counter--;
					this.log.debug(tag, `local.rt_counter: ${this.local.rt_counter}`);
				}
				...
				let json = {
					...
					"status_error": ansi(response_retweet.error),
					"status_warning": ansi(response_retweet.warning)
				};
				await this.api.database.insert(json);

				if (this.local.rt_counter < 1) {
					this.local.rt_counter = 12;
					this.log.info(tag, this.lang.translate("finish_fast_rt", {"time": `${this.utils.decimal_to_minutes(bot_seconds_wait_max / 60)}-${this.utils.decimal_to_minutes(bot_seconds_wait_min / 60)}`}));
...
module.exports = Rtmode_realistic;